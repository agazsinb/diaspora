 ...
 * @since Magbook 1.0
 */
class Magbook_Category_Control extends WP_Customize_Control {
	public $type = 'select';
	public function render_content() {
	$magbook_settings = magbook_get_theme_options();
	$magbook_categories = get_categories(); ?>
		<label>
			...
			<?php
				foreach ( $magbook_categories as $category) :?>
						<option value="<?php echo $category->cat_ID; ?>" <?php if ( in_array( $category->cat_ID, $magbook_settings) ) { echo 'selected="selected"';}?>><?php echo $category->cat_name; ?></option>
					<?php endforeach; ?>
			</select>
		</label>
	<?php 
...
$wp_customize->add_control(
	new Magbook_Category_Control(
	$wp_customize,
	'magbook_theme_options[magbook_small_slider_post_category]',
		array(
			'priority' 				=> 10,
